...
- **Email alerts** (`notifications.py`): built. Sends via Gmail SMTP (app password) using the same `keyring`-based `credentials.py` module as Monarch creds. Triggers: sync-completion summary (folds in the unmapped-categories warning rather than a separate email, since both come from the same sync event) and sync-failure alerts. Wired into the CLI (`sync --notify`, plus a standalone `test-email` subcommand) and the Streamlit app (Settings section to save Gmail creds + a "Send test email" button, and an "📧 Email me the results" checkbox in Sync). Unit-tested with a mocked `smtplib.SMTP` (`tests/test_notifications.py`) -- not yet verified against a real Gmail send.
- **Packaging** (`install.ps1`): a one-click PowerShell installer at the repo root -- installs Python via winget if missing, creates `.venv`, installs the package, seeds `config/owners.local.yaml` from the template if absent, and drops a Desktop shortcut that launches `streamlit run app.py` directly. Built so Andrew/Sharon never touch Python/venv themselves; syntax-validated but not yet run end-to-end on a clean machine.
- **Cash-to-keep prediction**: not started, still a later phase.
